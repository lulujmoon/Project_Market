			...
			<c:if test="${reviews.size() > 0 }">
				...
				<ul class="page-container list-page">
					<c:if test="${reviewPager.pre}">
						<li><a class="page-item arrow" href="${pageContext.request.contextPath}/store/${member.code}/myReviews?page=${reviewPager.startNum-1}"><i class="fas fa-angle-double-left"></i></a></li>
					</c:if>
					<c:forEach begin="${reviewPager.startNum}" end="${reviewPager.lastNum}" var="i">
							<li><a class="page-item code_${i}" href="${pageContext.request.contextPath}/store/${member.code}/myReviews?page=${i}">${i}</a></li>
					</c:forEach>
					<c:if test="${reviewPager.next}">
						<li><a class="page-item arrow" href="${pageContext.request.contextPath}/store/${member.code}/myReviews?page=${reviewPager.lastNum+1}"><i class="fas fa-angle-double-right"></i></a></li>
					</c:if>
				</ul>
			</c:if>
		</div>
